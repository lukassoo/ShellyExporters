...
public static class HttpServer
{
    ...
    static Func<string>? responseFunction;

    static HttpServer()
    {
        if (!HttpListener.IsSupported)
        {
            throw new Exception("The Http Server is somehow unsupported, read up what C# HttpListener supports");
        }

        httpListener = new HttpListener();
    }

    public static void SetResponseFunction(Func<string> responseFunctionRef)
    {
    ...
    static void StartRequestProcessing()
    {
        Task.Run(async () =>
        {
            ...
            while (true)
            {
                ...
                try
                {
                    response.OutputStream.Write(Encoding.UTF8.GetBytes(responseFunction()));
