...
internal static class Program
{
    ...
    static void SetupDevicesFromConfig(Config<TargetDevice> config)
    {
        ...
        foreach (TargetDevice target in config.targets)
        {
            ...
            devices.Add(new ShellyPlusPmMini(target));
        }
    }
    ...
    static void SetupDevicesWithOldNaming()
    {
        foreach (Device baseDevice in devices)
        {
            ...
            if (!device.IgnoreCurrentPower)
            {
                IMetric metric = MetricsHelper.CreateGauge(metricPrefix + "currently_used_power", "The amount of power currently flowing in watts", 
